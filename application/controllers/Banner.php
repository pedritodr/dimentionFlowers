...
class Banner extends CI_Controller
{
    ...
    public function add()
    {
        ...
        if ($allow_extension) {
            ...
            if ($result[0]) {
                $data = ['is_active' => 1, 'texto1' => "", 'texto2' => "", 'texto3' => "", 'imagen' => $result[1], 'url' => $url,  'orden' => $orden, 'tipo' => 0];
    ...
    public function update()
    {
        ...
        //  $tipo = $this->input->post('tipo');
        //establecer reglas de validacion
        $this->form_validation->set_rules('text1', translate('text_lang'), 'required');
        // $this->form_validation->set_rules('text2', translate('text_lang'), 'required');
          ...
          //en caso de que todo este bien
            ...
            if ($allow_extension || $_FILES['archivo']['error'] == 4) {
                if ($_FILES['archivo']['error'] == 4) {
                    $data = ['texto1' => "", 'texto2' => "", 'texto3' => "", 'orden'=>$orden, 'url' => $url];
                    ...
                    if ($banner_object) {
                        ...
                        if ($result[0]) {
                            ...
                            if (file_exists($banner_object->imagen))
                                unlink($banner_object->imagen);
                            $data = ['is_active' => 1, 'imagen' => $result[1], 'texto1' => "", 'texto2' => "", 'texto3' => "", 'orden'=> $orden, 'url' => $url];
                            $this->banner->update($banner_id, $data);
                            $this->response->set_message(translate("data_saved_ok"), ResponseMessage::SUCCESS);
                            redirect("banner/index", "location", 301);
                        } else {
