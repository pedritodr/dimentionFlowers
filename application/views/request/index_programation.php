<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <?= translate('manage_programation_lang'); ?>
            <small><?= translate('listar_programation_lang'); ?></small>
            | <a style="cursor:pointer" onclick="modal_programacion();" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?= "Buscar programación" ?>
            </a>

        </h1>
        <ol class="breadcrumb">
            <li><a href="<?= site_url('dashboard/index'); ?>"><i class="fa fa-dashboard"></i> <?= translate('pizarra_resumen_lang'); ?></a></li>
            <li class="active"><?= translate('listar_programation_lang'); ?></li>
            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><?= translate('listar_programation_lang'); ?></h3>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th><?= translate("date_vuelo_lang"); ?></th>
                                    <th><?= translate("datos_programation_lang"); ?></th>
                                    <th><?= translate("actions_lang"); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($request_date as $item) { ?>
                                    <tr>
                                        <td style="width:100px"><?= $item->date_time_reception; ?></td>

                                        <td style="width:80%">
                                            <?php foreach ($item->all_requests as $all_requests) { ?>
                                                <?php foreach ($all_requests->buy_element as $request_product) { ?>
                                                    <p><strong>Finca:</strong> <?= $request_product->provider ?>
                                                        <?php if ($request_product->etiqueta == NULL) {
                                                                        $producto = $request_product->product;
                                                                    } else {
                                                                        $producto = $request_product->etiqueta;
                                                                    } ?>
                                                        <strong>Variedad:</strong> <?= $producto ?>
                                                        <strong>Medida:</strong> <?= $request_product->measure ?>
                                                        <strong>Cajas:</strong> <?= $request_product->qty ?>
                                                        <strong>Tipo de caja:</strong> <?= $request_product->box ?>
                                                        <strong>Tallos:</strong> <?= $request_product->total_steams ?>
                                                        <strong>Marcación:</strong> <?= $request_product->dialing ?>
                                                        <strong>Destino:</strong> <?= $request_product->destination ?>
                                                        <strong>PO:</strong> <?= $request_product->purchase_order ?>
                                                    </p>
                                                    <?php if ($request_product->element) { ?>
                                                        <?php foreach ($request_product->element as $element) { ?>
                                                            <strong>Variedad:</strong> <?= $element->product ?>
                                                            <strong>Medida:</strong> <?= $element->name ?>
                                                            <strong>Tallos:</strong> <?= $element->nro_bunches * $element->stems_bunch ?>
                                                        <?php } ?>
                                                    <?php } ?>
                                        ...
                                        <td>
                                            <a href="<?= site_url('request/exportar_programation/' . $item->date_time_reception . "/" . $cliente_id); ?>" class="btn btn-success"><i class="fa fa-edit"></i> Exportar programación</a>
                                        </td>
                                    </tr>
