...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <small><?= translate('update_request_lang'); ?></small> | <a href="<?= site_url('request/index'); ?>" class="btn btn-default"><i class="fa fa-arrow-circle-left"></i> <?= translate('back_lang'); ?>
            </a>
        </h1>
        <ol class="breadcrumb">
            <li><a href="<?= site_url('dashboard/index'); ?>"><i class="fa fa-dashboard"></i> <?= translate('pizarra_resumen_lang'); ?></a></li>
            <li class="active"><?= translate('update_request_lang'); ?></li>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                        ...
                                        <div class="input-group input-group-sm">
                                            ...
                                            <select disabled id="clientes" name="clientes" class="form-control" data-placeholder="Seleccione una opción" style="width: 100%">
                                                <option value="0">Seleccione una opción</option>
                                    ...
                                    <?php foreach ($all_products as $item) { ?>
                                        <div id="contenedor_producto" class="col-xs-6 col-md-3 col-sm-4 col-lg-2 item">
                                            <div class='img-container'>
                                                ...
                                                ?>
                                                <button id="<?= $item->product_id; ?>" onclick="add_to_modal('<?= $item->product_category_id; ?>','<?= $item->product_id; ?>','<?= base64_encode($item->name); ?>','<?= $url; ?>','<?= $item->stems_bunch; ?>')" class="btn btn-success btn-add-cart"><span class=" glyphicon glyphicon-shopping-cart"></span> añadir</button>
...
<div class="modal fade" id="cantModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control input-sm" name="purchase" id="purchase" placeholder="<?= translate('purchase_order_lang'); ?>">
                                            </div>
                                        </div>
                                        <input name="request_id" id="request_id" type="hidden" value="">
                                        <div class="col-lg-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="date" class="form-control input-sm" id="date_purchase" name="date_purchase" required placeholder="<?= translate('date_delivery_lang'); ?>">
                                            </div>
                                        </div>
                                        <div class="col-lg-3">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("form")[0].reset();
        var cart = <?= json_encode($all_pedido); ?>;
        var pedido_object = <?= json_encode($pedido_object); ?>;
        var cliente = parseInt(pedido_object.cliente_id);

        $('#purchase').val(pedido_object.purchase_order);
        $('#date_purchase').val(pedido_object.date_purchase);
        $('#date_reception').val(pedido_object.date_time_reception);
        $('#request_id').val(pedido_object.request_id);
        ...
        if (cart) {
            if (cart.length > 0) {
                ...
                for (var i = 0; i < cart.length; i++) {
                    ...
                    addToCart(cart[i].product_category_id, cart[i].caja, cart[i].destino, cart[i].request_product_id, cart[i].product_id, cart[i].producto, cart[i].measure_id, cart[i].measure, cart[i].dialing, cart[i].box_type_id, cart[i].qty, cart[i].unit_price, cart[i].qty_bunches, cart[i].stems_bunch, cart[i].photo, cart[i].destination_id, cart[i].carguera_id, cart[i].total_price, cart[i].total_steams, 1);

                }

                destino = cart[cart.length - 1].destination_id;
                localStorage.setItem("destino", destino);
                marcacion = cart[cart.length - 1].dialing;
                localStorage.setItem("marcacion", marcacion);
                carguera = cart[cart.length - 1].carguera_id;
                localStorage.setItem("carguera", carguera);
            }
        }
    ...
    window.onload = function() {
        ...
        $('.btn-add').click((e) => {
            ...
            if ($("select[name=variety]").val() == 0) {
                ...
                let producto = eventTarget.parent().parent().find('h4')[0].textContent;
                let photo = eventTarget.parent().parent().find('img').attr('src');
                let product_id = eventTarget.parent().parent().find('#product_id')[0].textContent;
                let stems_bunch = eventTarget.parent().parent().find('#descriptions')[0].textContent;
                var cantidad_box = $('input[name=cantidadModal]').val();
                var price_unit = $('input[name=precioModal]').val();
                var qty_bunches = $('input[name=bunchesModal]').val();
                var dialing_id = $('select[name=marcacion]').val();
                var cliente_id = $('select[name=clientes]').val();
                var type_box = $('select[name=tipo]').val();
                var variety_id = $('input[name=variety_id]').val();
                var marcacion = $('input[name=marcacion]').val();
                var box_name = $('#tipo option:selected').text();
                ...
                if (elemento) {
                    ...
                    var cant_cajas = parseInt(elemento.attr('cantidad_box'));
                    //   updatePrice(0, sub, cajas);
                    itemsInCart--;
                    updatePrice(0, sub, cant_cajas);
                    elemento.remove();
                    elemento = 0;
                }
                request_product_id = $('#request_product_id').val();
                ...
                addToCart(category, box_name, destino, request_product_id, product_id, producto, measure_id, measure, marcacion, type_box, cantidad_box, price_unit, qty_bunches, stems_bunch, photo, destino_id, carguera_id, sub_total, total_bunches, 2)
    ...
    function add_to_modal(categoria, id, name, photo, bunch) {
        ...
        if (localStorage.getItem("destino")) {
            $("select[name=destination]").val(localStorage.getItem("destino"));
        }
        $('#titulo').hide();
        $('#destino').hide();
    ...
    function addToCart(category, box_name, destino, request_product_id, product_id, producto, measure_id, measure, marcacion, type_box, cantidad_box, price_unit, qty_bunches, stems_bunch, photo, destino_id, carguera_id, sub_total, total_steams, estatus) {
        ...
        var precio_caja = parseFloat(sub_total) / parseInt(cantidad_box);
        newDiv.text(cantidad_box + 'x' + precio_caja + '  ' + producto + ' ' + measure + ' ' + '  subTotal  ' + sub_total + ' ');
        ...
        if (estatus == 2 && request_product_id > 0) {
            newDiv.append('<button class="btn btn-danger btn-xs" onclick="deleteItem(this)">X</button> <button class="btn btn-info btn-xs" onclick="editar(this)">Editar</button>');
        }
        if (estatus == 1 && request_product_id > 0) {
            newDiv.append('<button class="btn btn-danger btn-xs" onclick="deleteItem(this)">X</button> <button class="btn btn-info btn-xs" onclick="editar(this)">Editar</button>');
        }

        newDiv.append('<hr>');
        newDiv.attr('request_product_id', request_product_id);
        newDiv.attr('product_id', product_id);
        newDiv.attr('producto', producto);
        newDiv.attr('measure_id', measure_id);
        newDiv.attr('measure', measure);
        newDiv.attr('marcacion', marcacion);
        newDiv.attr('type_box', type_box);
        newDiv.attr('cantidad_box', cantidad_box);
        newDiv.attr('price_unit', price_unit);
        newDiv.attr('qty_bunches', qty_bunches);
        ...
        newDiv.attr('sub_total', sub_total);
        newDiv.attr('total_steams', total_steams);
        newDiv.attr('destino', destino);
        newDiv.attr('box_name', box_name);
        newDiv.attr('category', category);
        if (estatus == 1) {
            newDiv.attr('estatus', 1);
        } else {
            newDiv.attr('estatus', 2);
        }
        ...
        updatePrice(sub, 0, cantidad_box);

    }

    function deleteItem(e) {
        let sub_total = $(e.parentElement).attr('sub_total');
        let cantidad = $(e.parentElement).attr('cantidad_box');
    ...
    function editar(e) {
        ...
        $('#cantModal').modal("show");
        var cant = parseInt($(e.parentElement).attr('cantidad_box'));
        total_bunches = (parseInt($(e.parentElement).attr('total_steams')));
        var tallos = total_bunches * cant;
        $('#tallos').text(tallos);
        ...
        $('#cantidadModal').val($(e.parentElement).attr('cantidad_box'));
        $('#precioModal').val($(e.parentElement).attr('price_unit'));
        $('#bunchesModal').val($(e.parentElement).attr('qty_bunches'));
        $('#marcacion').val($(e.parentElement).attr('marcacion'));
        $("select[name=tipo]").val($(e.parentElement).attr('type_box'));
    ...
    function cartToString() {
        ...
        for (let i = 0; i < cartItems.length; i++) {
            ...
            arreglo.push({
                "request_product_id": cartItems[i].getAttribute('request_product_id'),
                "product_id": cartItems[i].getAttribute('product_id'),
                "variety_measure_id": cartItems[i].getAttribute('measure_id'),
                "qty_bunches": cartItems[i].getAttribute('qty_bunches'),
                "measure": cartItems[i].getAttribute('measure'),
                "nombre": cartItems[i].getAttribute('producto'),
                "destino": cartItems[i].getAttribute('destino'),
                "cantidad": cartItems[i].getAttribute('cantidad_box'),
                "precio": cartItems[i].getAttribute('price_unit'),
                "foto": cartItems[i].getAttribute('photo'),
                "subtotal": cartItems[i].getAttribute('sub_total'),
                "tipo_id": cartItems[i].getAttribute('type_box'),
                "tipo_name": cartItems[i].getAttribute('box_name'),
                "dialing_name": cartItems[i].getAttribute('marcacion'),
                "total_bunches": cartItems[i].getAttribute('total_steams'),
    ...
    $("#formulario").submit(function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "<?= site_url('request/edit_request') ?>",
            data: {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        ...
                        var cadena_button = "<button id=btn_" + result[i].product_id + " onclick='' class='btn btn-success btn-add-cart'><span class=' glyphicon glyphicon-shopping-cart'></span> añadir</button><img style='width:200px; height:167px;' src='" + photo + "'>";
